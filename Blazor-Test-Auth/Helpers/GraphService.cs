    ...
    public static class GraphService
    {
        public static async Task<Stream> GetMyPictureStream(GraphServiceClient graphClient, HttpContext httpContext)
        {
            ...
            try
            {
                ...
                {
                    if (e.Error.Code == "GetUserPhoto")
                    {
                        // User is using MSA, we need to use beta endpoint
        ...
        public static async Task<string> GetPictureBase64(
                ...
                switch (e.Message)
                {
                    ...
                    case "EmailIsNull":
                        return JsonConvert.SerializeObject(
                            new { Message = "Email address cannot be null." },
                            Formatting.Indented);
                    default:
                        return null;
                }
        ...
        public static async Task<Stream> GetPictureStream(
            GraphServiceClient graphClient,
            string email,
            HttpContext httpContext)
        {
        ...
        public static async Task<string> GetUserJson(
            GraphServiceClient graphClient,
            string email,
            HttpContext httpContext)
        {
            ...
            catch (ServiceException e)
            {
                switch (e.Error.Code)
                {
                    ...
                    case "itemNotFound":
                        return JsonConvert.SerializeObject(
                            new { Message = $"User '{email}' was not found." },
                            Formatting.Indented);
                    case "ErrorInvalidUser":
                        return JsonConvert.SerializeObject(
                            new { Message = $"The requested user '{email}' is invalid." },
                            Formatting.Indented);
                    ...
                    default:
                        return JsonConvert.SerializeObject(
                            new { Message = "An unknown error has occurred." },
                            Formatting.Indented);
                }
            }
        }
            ...
            HttpContext httpContext)
        {
            ...
            var recipientList = splitRecipientsString.Select(
                recipient => new Recipient { EmailAddress = new EmailAddress { Address = recipient.Trim() } }).ToList();
                                ...
                                Body = new ItemBody
                                           {
                                               Content = File.ReadAllText(
                                                   hostingEnvironment.WebRootPath + "/email_template.html"),
